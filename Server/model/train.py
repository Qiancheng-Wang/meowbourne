...
def generate_labels():
    ...
    with open('Server/model/labels.csv', 'w', newline='') as csvfile:

        writer = csv.writer(csvfile)
        writer.writerow(["id", "breed"])

        for filename in os.listdir("Server/model/train"):
            # print(filename)
            name_arr = filename.replace('_',' ').replace('.',' ').split()
            #print(name_arr[])
            name_arr = name_arr[:len(name_arr)-2]
...
def train():
    ...
    ## Training
    callbacks = [
        ModelCheckpoint(path + "model.h5", verbose=1, save_best_only=True),
